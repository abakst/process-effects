module Language.Haskell.MessagePassing.Effects ( synthEffects ) where
...
import Language.Haskell.MessagePassing.EffEnv
import Language.Haskell.MessagePassing.Builtins
import Language.Haskell.MessagePassing.PrettyPrint
...
synthEff g e@(Var x)
  = do effEnv     <- First        <$> lookupEffTy g x
       ...
       let e = fromMaybe err . getFirst $ effEnv <> builtinEff <> def
       freshFnEff e
  where
